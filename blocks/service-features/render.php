...
foreach ( $features_raw as $feat ) {
	...
	// ACF repeater: bullets is array of arrays with 'text' key
	if ( ! empty( $bullets ) && isset( $bullets[0]['text'] ) ) {
		$bullets = array_map( function( $b ) { return $b['text'] ?? $b; }, $bullets );
	}
	$img_url = '';
	if ( ! empty( $feat['image'] ) ) {
		if ( is_array( $feat['image'] ) ) {
			$img_url = $feat['image']['url'] ?? '';
		} elseif ( is_numeric( $feat['image'] ) ) {
			$img_url = wp_get_attachment_url( $feat['image'] );
		} else {
			$img_url = $feat['image'];
		}
	}
